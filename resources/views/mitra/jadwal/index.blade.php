<x-app-layout>
    <div class="py-8 px-6 lg:px-8 max-w-7xl mx-auto bg-gray-50 min-h-screen">
        ...
        {{-- Tempat Nampilin Pesan Sukses / Error --}}
        @if(session('success'))
            ...
            <table class="w-full text-center border-collapse">
                <thead>
                    <tr class="bg-blue-100/50 text-gray-600 text-sm border-b border-gray-200">
                        <th class="py-4 px-4 font-bold">ID Slot</th>
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="py-4 px-4 text-sm">
                                @if($slot->status == 'siap dipakai')
                                    <span class="text-green-600 font-bold bg-green-100 px-3 py-1 rounded-full text-xs">Siap
                                        Dipakai</span>
                                @elseif($slot->status == 'maintenance')
                                    <span
                                        class="text-orange-500 font-bold bg-orange-100 px-3 py-1 rounded-full text-xs">Maintenance</span>
                                @else
                                    <span
                                        class="text-red-500 font-bold bg-red-100 px-3 py-1 rounded-full text-xs">Di-booking</span>
                                @endif
                            </td>
                            <td class="py-4 px-4">
                                @php
            ...
            <div class="bg-white rounded-xl shadow-lg w-full max-w-md p-6 relative">
                ...
                <form action="{{ route('mitra.jadwal.update') }}" method="POST">
                    ...
                    <div class="mb-6">
                        ...
                        <p class="text-xs text-orange-500 font-medium mt-2">*Pilih Maintenance untuk memblokir jadwal
                            ini agar tidak bisa dibooking penyewa.</p>
                    </div>
